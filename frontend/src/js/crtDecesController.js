...
// Acte de naissance
const crtDecesForm = document.getElementById('crtDecesForm');
crtDecesForm.addEventListener('submit', async e => {
    e.preventDefault();
    ...
    demande_body.append('type_document_id', 3);
    ...
    try {
        ...
        if (response.ok) {
            ...
            const url_acte = 'http://localhost:8000/api/certificats-de-dece'
            const formData = new FormData(crtDecesForm)
