...
// Acte de mariage
const demandeActeMariageForm = document.getElementById('demandeActeMariageForm');
demandeActeMariageForm.addEventListener('submit', async e => {
    e.preventDefault();
    ...
    demande_body.append('type_document_id', 2);
    ...
    try {
        ...
        if (response.ok) {
            ...
            const url_acte = 'http://localhost:8000/api/actes-de-mariage'
            const formData = new FormData(demandeActeMariageForm)
            ...
            if (responseSecond.ok) {
                ...
                showSuccessToast('Demande d\'acte de mariage soumise et traitement réussi.');
            } else {
